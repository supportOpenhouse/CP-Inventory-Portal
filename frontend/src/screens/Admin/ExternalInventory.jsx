...
// All rows up to and including the column-header row stay frozen as the body scrolls.
const HEADER_HEIGHT      = 56;   // page header (Back + title)
const SEARCH_ROW_HEIGHT  = 56;   // search + type toggle + counts
const FILTER1_HEIGHT     = 56;   // city / source / bhk / floor / area (sqft)
const FILTER2_HEIGHT     = 56;   // date chips + custom range
const HEADERS_TOP        = HEADER_HEIGHT + SEARCH_ROW_HEIGHT + FILTER1_HEIGHT + FILTER2_HEIGHT;
...
export default function ExternalInventory({ onClose }) {
  ...
  return (
    <div className="app-shell" style={{ maxWidth: 'none' }}>
      ...
      {/* Filter row 1 — city / source / bhk / floor / area. Sticky just below search. */}
      <div style={{
        position: 'sticky', top: HEADER_HEIGHT + SEARCH_ROW_HEIGHT, zIndex: 22,
        display: 'flex', gap: 14, padding: '10px 20px',
        alignItems: 'center', flexWrap: 'nowrap',
        borderBottom: '1px solid #eee', background: '#fff',
        height: FILTER1_HEIGHT, boxSizing: 'border-box', fontSize: 13,
        overflowX: 'auto',
      }}>
      ...
      {/* Filter row 2 — date chips + custom range. Sticky below filter row 1. */}
      <div style={{
        position: 'sticky', top: HEADER_HEIGHT + SEARCH_ROW_HEIGHT + FILTER1_HEIGHT, zIndex: 20,
        display: 'flex', gap: 14, padding: '10px 20px',
        alignItems: 'center', flexWrap: 'nowrap',
        borderBottom: '1px solid #eee', background: '#fff',
        height: FILTER2_HEIGHT, boxSizing: 'border-box', fontSize: 13,
        overflowX: 'auto',
      }}>
            ...
            {COLUMNS.map((c) => (
              <th
                ...
                style={{
                  ...
                  userSelect: 'none',
                  background: (c.sortable && sortCol === c.key) ? '#FF6B2B' : '#1a1a2e',
                  color: '#fff',
                }}
...
// Column-header row — visually heavy on purpose so it reads as a clear
// "this is where the data table starts" delimiter under the (lighter)
// filter bars above. Dark background + bright text = strong contrast.
// Active sort column is set in render (via `style={{ background: ... }}`)
// to a slightly different shade so the active column still stands out
...
const thStyle = {
  ...
  top: HEADERS_TOP,
  zIndex: 15,
  background: '#1a1a2e',  // matches the dark navy used in the CP-side header
};
